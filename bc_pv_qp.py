...
def bcpv(num_suppliers, num_buyers, num_parking_zones, zij, xij, gcs, g_c, bj, si):
    cpx = cplex.Cplex()
...
def bcpv_s1(num_suppliers, num_buyers, num_parking_zones, zij, xij, gcs, g_c, bj, si, padding, si_coff, padding_num):
    cpx = cplex.Cplex()
    cpx.objective.set_sense(cpx.objective.sense.maximize)
    # varaibles
    ebs = [None] * num_parking_zones
...
def bcpv_s2(num_suppliers, num_buyers, num_parking_zones, zij, xij, gcs, g_c, bj, si, padding, si_coff, padding_num, b_id):
    cpx = cplex.Cplex()
    cpx.objective.set_sense(cpx.objective.sense.maximize)
    # varaibles
    ebs = [None] * num_parking_zones
...
def bcpv_s3(num_suppliers, num_buyers, num_parking_zones, zij_n, xij, gcs, g_c, bj, si):
    cpx = cplex.Cplex()
    cpx.objective.set_sense(cpx.objective.sense.maximize)
    # varaibles
    ebs = [None] * num_parking_zones
...
def bcpv_s4(num_suppliers, num_buyers, num_parking_zones, zij_n, xij, gcs, g_c, bj, si, s_id):
    cpx = cplex.Cplex()
    cpx.objective.set_sense(cpx.objective.sense.maximize)
    # varaibles
    ebs = [None] * num_parking_zones
...
def run_bcpv(num_suppliers, num_buyers, num_parking_zones, data, padding):
    # utility_s, utility_b, pay, ext_cost, max sw, sw, time, efficency, trade num
    bcpv_output = [0] * 9
    bcpv_start_time = time.time()
    # esc
    bcpv_r1 = bcpv_s1(num_suppliers,
                      num_buyers,
                      num_parking_zones,
    ...
    ebs_b_org = np.array(bcpv_r1[2:]).reshape(num_parking_zones, num_buyers)
    ebs_b = ebs_b_org.sum(axis=0)
    for i in range(num_buyers):
        if ebs_b[i] > 0.99:
            bcpv_r2 = bcpv_s2(num_suppliers,
                              num_buyers,
                              num_parking_zones,
                              data.zij, data.xij, data.gcs, data.g_c,
                              data.bj, data.si, data.padding_v, data.si_coff,
                              data.padding_num, i)
            bcpv_output[1] += bcpv_r1[1] - bcpv_r2[1]
            b_price = math.floor(data.bj[i]) - (bcpv_r1[1] - bcpv_r2[1])
            bcpv_output[2] += b_price
        else:
            for j in range(num_parking_zones):
                zij_n[j][i] = 0
    # allocation
    bcpv_r3 = bcpv_s3(num_suppliers,
                      num_buyers,
                      num_parking_zones,
    ...
    for i in range(num_suppliers):
        if eta_s[i] > 0.99:
            bcpv_r4 = bcpv_s4(num_suppliers,
                              num_buyers,
                              num_parking_zones,
                              zij_n, data.xij, data.gcs, data.g_c,
                              data.bj, data.si, i)
            bcpv_output[0] += bcpv_r3[1] - bcpv_r4[1]
            s_price = math.ceil(data.si[i]) + (bcpv_r3[1] - bcpv_r4[1])
            bcpv_output[2] -= s_price
    bcpv_r = bcpv(num_suppliers,
                  num_buyers,
                  num_parking_zones,
                  data.zij, data.xij, data.gcs, data.g_c,
                  data.bj, data.si)
    ext_num = np.array(eta_s).reshape((num_parking_zones, -1)).sum(axis=1)
    bcpv_output[3] = (ext_num*ext_num*data.g_c_2).sum()
    bcpv_output[4] = bcpv_r[1]
    bcpv_output[5] = bcpv_r3[1]
    bcpv_output[6] = time.time() - bcpv_start_time
    bcpv_output[7] = bcpv_output[5] * 1.0 / bcpv_output[4]
    bcpv_output[8] = sum(eta_s)
    return bcpv_output
...
if __name__ == "__main__":
    ...
    result = run_bcpv(num_suppliers, num_buyers, num_parking_zones, data, padding)
    print(1)
